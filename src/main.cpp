...
#if defined(NDEBUG)
# error "Nhucoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x0000099c56af6b840bd9b2485d179f725dbaca96659d9858948bfbe8a177bea1");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Nhucoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Nhucoin Signed Message:\n";
...
{
    // Nhucoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 250000); // Nhucoin: 250k blocks in ~1 year

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Nhucoin: 1 day
static const int64 nTargetSpacing = 2 * 60; // Nhucoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Nhucoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Nhucoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        block.nVersion = 1;
        block.nTime    = 1414202219;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2088560731;

        if (fTestNet)
        {
            block.nTime    = 1414202158;
        ...
        assert(block.hashMerkleRoot == uint256("0x2757a0d912991ee084f0226d8b479ab653b35a9831d6b500c1d6bd82cae907c5"));
...
unsigned char pchMessageStart[4] = { 0xf4, 0xc3, 0xb2, 0xd1 };
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("NhucoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("NhucoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static NhucoinMiner(CWallet *pwallet)
{
    printf("NhucoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("nhucoin-miner");
    ...
    try { loop {
        ...
        printf("Running NhucoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("NhucoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&NhucoinMiner, pwallet));
}
